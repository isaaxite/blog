...
!function(){
	...
	ImageHandle.prototype.lazyLoad = function(callback) {
		var that = this;
		var options = this.options;

		// callback容错
		if(!$.isFunction(callback) && typeof callback != 'undefined'){
			console.error('Please Init \'lazyLoad\' With Callback Function!');
			return false;
		}

		$.each($(options.el), function(i, v) {
			var img = new Image();
			var $img = $(v);
			var origin = $img.attr(options.origin)
			img.src = origin;

			if(img.complete) {
				that.imgOperate($img, origin, callback);
			}else{
				img.onload = function() {
					that.imgOperate($img, origin, callback);
				}
	...
	ImageHandle.prototype.imgOperate = function($img, origin, callback) {
