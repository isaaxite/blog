...
本文系统地总结了 JavaScript 中六种继承方式的实现原理、代码示例与核心问题：

- **原型链继承**：通过将子类原型指向父类实例实现，但引用属性会被所有实例共享；
- **构造函数继承**：在子类中调用父类构造函数解决引用共享问题，但无法继承父类原型方法；
- **组合继承**：结合两者优点，用构造函数继承属性、原型链继承方法，但会两次调用父类构造函数；
- **原型式继承**：基于现有对象创建新对象（`Object.create()`），适合无需构造函数的场景；
- **寄生式继承**：在原型式继承基础上增强对象，添加额外方法；
- **寄生组合式继承**：通过复制父类原型避免多余构造调用，是最理想的继承方式。
