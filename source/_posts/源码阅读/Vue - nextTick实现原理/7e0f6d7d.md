---
title: nextTick实现原理
slugpath: source-code-reading/vue/nextTick
alias: blog/source-vue-nextTick/
...
进入 nextTick，首先会对 cb 进行封装，涉及三个控制流：

1. 如果存在 cb，则调用 `cb.call(ctx)`，改变 `cb` 的上下文；
2. 如果 `_resolve` （`_resolve` 是 `Promise.resolve` 的引用！前提是当前环境支持 `Promise`）存在，则调用 `_resolve(ctx)`；
3. 不做任何处理。

在将 `cb` 推入 `callbacks` 后，判断当前是不是正在执行上次`callbacks` 的回调函数，根据 pending（待定）来判断，当前是否要执行新的 `callbacks` 的 cb！
