---
title: Vue 源码分析 - 深入剖析计算属性的运行原理
slugpath: source-code-reading/vue/computed-properties
alias: blog/vue-source-computed-properties-deep-dive/
excerpt: "Vue 计算属性源码阅读，分析其初始化、被访问时、更新时的运行过程。更新时是如何通知变动、更新的。计算属性的依赖是如何收集的。"
...
创建 Vue 实例时调用 `this._init` 进行初始化。其中之一就有调用 `initState`，而它内部会调用 `initComputed` 初始化计算属性：
...
在 `initComputed` 内部，会遍历 computed 得到它的 `key`。在 `watchers` 这个数据內新的映射关系——以为 `computed` 的 `key` 键名，以计算属性类型的 watcher 实例为键值。

*注意：`computedWatcherOptions` 这个 option 决定了计算属性的 watcher 和普通 watcher 的不同。*

然后定义计算属性的属性的 getter 和 setter
...
![执行完`computed-watcher.get()`后的打印信息](https://user-images.githubusercontent.com/25907273/62833255-53ced880-bc6e-11e9-8c07-722e85c93e62.png)
